...
def detect_and_log_slow_tests(
    ...
    if slow_tests:
        ...
        slow_msg = f"🐌 Found {len(slow_tests)} slow test(s) " f"(> {slow_threshold}s):"
...
def detect_and_log_flaky_tests(
    ...
    if enable_flaky_detection:
        ...
        # Report flaky tests
        if flaky_tests:
            flaky_msg = f"⚠️  Found {len(flaky_tests)} " f"potentially flaky test(s):"
...
def build_output_with_failures(
    summary_data: dict,
    failed_issues: list[PytestIssue],
) -> str:
    """Build output string with summary and failure details.
