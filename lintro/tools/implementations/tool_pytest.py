...
class PytestTool(BaseTool):
    ...
    def __post_init__(self) -> None:
        ...
        self.executor = PytestExecutor(self.pytest_config)
        self.executor.tool = self  # Set tool reference for subprocess execution
    ...
    def _parse_output(
        ...
        return parse_pytest_output_with_fallback(
            ...
            options=self.pytest_config.get_options_dict(),
        )

    def _get_total_test_count(self, target_files: list[str]) -> int:
        """Get total count of all available tests.

        Backward compatibility method that delegates to collect_tests_once.
        ...
        total_available_tests, _ = collect_tests_once(None, target_files)
    ...
    def _count_docker_tests(self, target_files: list[str]) -> int:
        ...
        _, docker_test_count = collect_tests_once(None, target_files)
