...
def test_command(
    ...
    if tool_options:
        ...
        while i < len(parts):
            ...
            # or starts with pytest: (already prefixed)
            if "=" in current_part or current_part.startswith("pytest:"):
                # Complete option
                if not current_part.startswith("pytest:"):
                    prefixed_options.append(f"pytest:{current_part}")
                else:
                    prefixed_options.append(current_part)
                i += 1
            else:
