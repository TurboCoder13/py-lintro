...
class ConsoleLogger:
    ...
    def save_console_log(self) -> None:
        ...
        Creates an empty console.log file in the run directory.
        """
        if self.run_dir:
    ...
    def _print_metadata_messages(self, raw_output: str) -> None:
        ...
        # Look for fixable issues
        fixable_match = re.search(r"(\d+)\s*fixable", raw_output, re.IGNORECASE)
        if fixable_match:
            fixable_count = int(fixable_match.group(1))
            if fixable_count > 0:
                self.console_output(text=f"Auto-fixable issues: {fixable_count}")
            else:
                self.console_output(text="No issues found")
        elif "cannot be auto-fixed" in raw_output.lower():
            # If there are non-fixable issues, show that
            self.console_output(text="Found issues that cannot be auto-fixed")
        elif "would reformat" in raw_output.lower():
            # If there are formatting suggestions, show that
            self.console_output(text="Would reformat files")
        elif "fixed" in raw_output.lower():
            # If issues were fixed, show that
            self.console_output(text="Issues were fixed")
        else:
            # If no specific patterns found, assume no issues
            self.console_output(text="No issues found")
