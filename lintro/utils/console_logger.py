            ...
            for result in tool_results:
                ...
                if action == "test" and tool_name.lower() in ("pt", "pytest"):
                    ...
                    if pytest_summary:
                        ...
                        passed = int(_summary_val(pytest_summary, "passed", 0))
                        failed = int(_summary_val(pytest_summary, "failed", 0))
                        skipped = int(_summary_val(pytest_summary, "skipped", 0))
                        docker_skipped = int(
                            _summary_val(pytest_summary, "docker_skipped", 0),
                        )
                        duration = float(_summary_val(pytest_summary, "duration", 0.0))
                        total = int(_summary_val(pytest_summary, "total", 0))
