...
def _get_summary_value(pytest_summary: dict[str, Any], key: str, default: Any) -> Any:
    """Get a value from pytest summary with fallback.

    Args:
        pytest_summary: The pytest summary dictionary
        key: The key to look for
        default: Default value if key not found

    Returns:
        The value from the summary or the default
    """
    return get_summary_value(pytest_summary, key, default)
...
def print_summary_table(
    ...
    try:
        ...
        for result in tool_results:
            ...
            if action == Action.TEST and tool_name.lower() == ToolName.PYTEST:
                pytest_summary = getattr(result, "pytest_summary", None)
                if pytest_summary:
                    # Use pytest summary data for more detailed display
                    passed = int(
                        _get_summary_value(pytest_summary, "passed", 0),
                    )
                    failed = int(
                        _get_summary_value(pytest_summary, "failed", 0),
                    )
                    skipped = int(
                        _get_summary_value(pytest_summary, "skipped", 0),
                    )
                    docker_skipped = int(
                        _get_summary_value(
                            pytest_summary,
                    ...
                    duration = float(
                        _get_summary_value(pytest_summary, "duration", 0.0),
                    )
                    total = int(
                        _get_summary_value(pytest_summary, "total", 0),
                    )
