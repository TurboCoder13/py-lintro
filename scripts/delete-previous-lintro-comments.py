...
This script deletes all comments containing the marker '<!-- lintro-report -->' from a specified pull request.
...
def delete_comment(repo: str, comment_id: int, token: str) -> None:
    ...
    with httpx.Client(timeout=10) as client:
        ...
        if response.status_code == 204:
            ...
            print(
                f"Failed to delete comment {comment_id}: {response.status_code} {response.text}",
                file=sys.stderr,
            )
