...
if [ -z "$COVERAGE_PERCENTAGE" ]; then
    if [ -f "coverage.xml" ]; then
        COVERAGE_PERCENTAGE=$(uv run python scripts/extract-coverage.py 2>&1 | grep "percentage=" | tail -1 | cut -d'=' -f2)
...
# Only commit if we're in CI and have git configured
if [ -n "$CI" ] || [ -n "$GITHUB_ACTIONS" ]; then
    ...
    log_info "Not in CI environment, skipping git operations"
    log_success "Coverage badge generated locally: coverage-badge.svg"
fi 