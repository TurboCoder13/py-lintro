# Script Organization & Responsibility Boundaries

This document defines clear responsibility boundaries for script organization following SOLID principles.
...
### Completed ✅
- Split `bootstrap-env.sh` into focused components
- Created `coverage-manager.sh` with subcommands
- Standardized script interfaces (--help, --dry-run, --verbose)

### In Progress 🟠
- Document organization boundaries (this file)
- Audit existing scripts for boundary violations

### Planned ❌
- Move `scripts/local/update-coverage-badge.sh` → use `coverage-manager.sh badge`
- Extract common functionality from CI scripts to utils
- Standardize remaining script interfaces
