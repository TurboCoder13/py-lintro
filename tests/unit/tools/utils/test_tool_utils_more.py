...
def test_format_tool_output_with_parsed_issues_and_fixable_sections(
    ...
    from lintro.utils import output_formatting

    monkeypatch.setattr(output_formatting, "TABULATE_AVAILABLE", True, raising=True)
    monkeypatch.setattr(output_formatting, "tabulate", fake_tabulate, raising=True)
    issues = [
        RuffIssue(
            file="a.py",
            line=1,
...
def test_format_tool_output_pytest_raw_fallback(monkeypatch) -> None:
    ...
    monkeypatch.setattr(output_formatting, "TABULATE_AVAILABLE", False, raising=True)
